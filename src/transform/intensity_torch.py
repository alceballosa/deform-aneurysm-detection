    ...
    def __call__(self, image):
        ...
        for i in range(gamma_level.shape[0]):
            if random.random() < 0.5:
                gamma_level[i] = 1.0 / gamma_level[i]

        gamma_level[apply == 0] = 1.0
        noise_image = torch.pow(
            image[:, self.channel_apply], gamma_level.to(image.device)
        )
        t_image = image.clone()
...
class RandomNoise(nn.Module):
    ...
    def __call__(self, image):
        ...
        noise_level = (
            torch.rand(image.shape[0], 1, 1, 1, device=image.device)
            * (self.gamma_range[1] - self.gamma_range[0])
            + self.gamma_range[0]
        )
        noise = (
            torch.randn(image.shape, device=image.device)[:, 0]
            * noise_level
            * apply.float()
        )
