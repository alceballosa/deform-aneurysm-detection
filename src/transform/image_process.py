...
def show_box2(volume, point, skip=1, width=50):
    ...
    for i in range(3):
        plt.subplot(1, 3, i + 1)
        plt.imshow(
            volume[z[i], point[1] - w : point[1] + w, point[2] - w : point[2] + w],
            cmap="gray",
            vmin=0,
            vmax=1,
        )
...
def fill_crop(img, min_idx, max_idx):
    ...
    *Note:* negative values in `pos` are interpreted as-is, not as "from the end".
    """
    crop = np.zeros(
        np.array(max_idx, dtype="int16") - np.array(min_idx, dtype="int16"),
        dtype=img.dtype,
    )
    img_shape, start, crop_shape = (
        np.array(img.shape),
        np.array(min_idx, dtype="int16"),
        np.array(crop.shape),
    )
...
def crop_ND_with_center_coord(coord, center_idx, output_size):
    ...
    coord = np.array(
        [loc for loc in coord if ((loc < max_idx).all() and (loc >= min_idx).all())]
    ).reshape([-1, 3])
    coord = coord - min_idx
    return coord
...
def crop_ND_volume_with_bounding_box(volume, min_idx, max_idx):
    ...
    if dim == 2:
        output = volume[
            np.ix_(range(min_idx[0], max_idx[0]), range(min_idx[1], max_idx[1]))
        ]
    elif dim == 3:
        output = volume[
            np.ix_(
                range(min_idx[0], max_idx[0]),
...
def set_ND_volume_roi_with_bounding_box_range(
    volume, bb_min, bb_max, sub_volume, addition=True
):
    """
    ...
    if dim == 2:
        if addition:
            out[
                np.ix_(range(bb_min[0], bb_max[0]), range(bb_min[1], bb_max[1]))
            ] += sub_volume
        else:
            out[
                np.ix_(range(bb_min[0], bb_max[0]), range(bb_min[1], bb_max[1]))
