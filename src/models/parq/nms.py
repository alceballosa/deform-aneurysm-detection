...
def nms(
    pred_corners, labels, scores, num_semcls, overlap_threshold, nms_type="nms_3d_faster_samecls"
):
...
def run_nms(
    ...
    pred_mask = np.zeros((bs, K)).astype(bool)
    ...
    for i in range(bs):
        ...
        boxes_3d_with_prob[:, 6] = logits[i, :, 0] # score for aneyrusm
        boxes_3d_with_prob[:, 7] = labels[i, :]  # only suppress if the two boxes are of the same class!!
        # print(np.mean(boxes_3d_with_prob, axis = 0))
        bg_box_inds = np.where(labels[i, :] != num_semcls)[0]
