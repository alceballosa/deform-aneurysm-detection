...
# https://github.com/uni-medical/SAM-Med3D/blob/main/segment_anything/modeling/image_encoder3D.py
...
class SAM3D_Encoder(nn.Module):
    def __init__(
        ...
        resize_patch_projection: bool = False,
        resize_patch_factor: int = 8,
    ) -> None:
        """
        Args:
        ...
        self.path_weights = pretrained_encoder_path
        ...
        if self.resize_patch_projection:
            patch_size = resize_patch_factor  # 8 for all _exp experiments
        ...
        if self.use_pretrained_encoder and self.path_weights != "":
    ...
    def load_pretrained_encoder_weights(self):
        ...
        """
        weights = torch.load(self.path_weights, map_location=torch.device("cpu"))[
            "model_state_dict"
        ]
        list_omit = []
        if self.resize_patch_projection:
            list_omit += ["pos_embed", "patch_embed.proj.weight"]
            list_omit += ["blocks.{}.attn.rel_pos_d".format(i) for i in [2, 5, 8, 11]]
            list_omit += ["blocks.{}.attn.rel_pos_h".format(i) for i in [2, 5, 8, 11]]
            list_omit += ["blocks.{}.attn.rel_pos_w".format(i) for i in [2, 5, 8, 11]]
        model_keys = list(weights.keys())
        remapped_model_keys = {
            key.replace("image_encoder.", ""): weights[key]
            for key in model_keys
            if "image_encoder" in key
        }
        remapped_model_keys = OrderedDict(remapped_model_keys)
        model_keys = list(self.state_dict().keys())
        # remove keys not in model_keys
        remapped_model_keys = {
            model_key: remapped_model_keys[model_key]
            for model_key in model_keys
            if model_key in remapped_model_keys and model_key not in list_omit
        }
        list_keys = list(remapped_model_keys.keys())
        self.frozen_parameters_list.extend(list_keys)
        self.load_state_dict(remapped_model_keys, strict=False)
        print(
            "\nLoaded pretrained SAM3D weights"
            + f"from {self.path_weights} successfully"
        )
        return
