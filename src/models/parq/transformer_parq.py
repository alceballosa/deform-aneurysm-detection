...
class TransformerDecoderLayer(nn.Module):
    def __init__(
        ...
        if use_flash_attn:
            ...
            self.multihead_attn = MHA(
                d_model, nhead, cross_attn=True, dropout=dropout
            )
        else:
    ...
    def forward_post(
        ...
        query_tokens_attended, attn = self.multihead_attn(
            query=self.with_pos_embed(query_tokens, query_pos),
            key=self.with_pos_embed(global_tokens, global_pos),
            value=global_tokens,
            attn_mask=global_tokens_mask,
            key_padding_mask=global_tokens_key_padding_mask,
        )
    ...
    def forward(
        ...
        if self.use_flash_attn:
            return self.forward_post_flash(
                query_tokens,
                global_tokens,
                query_mask,
                global_tokens_mask,
