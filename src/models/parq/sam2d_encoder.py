...
class Adapter_Layer(nn.Module):
    def __init__(
        self, embed_dim, mlp_ratio=0.25, norm_layer=nn.LayerNorm, skip_connect=True
    ):
        super().__init__()
        ...
        self.spatial = nn.Sequential(
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(
                embed_dim, embed_dim, kernel_size=4, stride=2, padding=1, bias=False
            ),
            nn.ReLU(),
        )
...
class SAM2D_Encoder(nn.Module):
    def __init__(
        ...
        self.depth_downsample_factor = 8
    ...
    def load_pretrained_encoder_weights(self):
        ...
        """
        weights = torch.load(
            self.path_weights, map_location=torch.device("cpu")
        )["model"]
        model_keys = list(weights.keys())
        remapped_model_keys = {
            key.replace("image_encoder.", ""): weights[key]
            for key in model_keys if "image_encoder" in key
        }
        ...
        remapped_model_keys = {
            ...
            if model_key in remapped_model_keys
        }
        ...
        print(
            "\nLoaded pretrained SAM2D weights"
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.pos_embed is not None:
            x = x + self.pos_embed
        x = einops.rearrange(x, "(b d) c h w -> b c d h w", b = B, d = D)
        x = x[:, :, torch.arange(0, 256, self.depth_downsample_factor), :, :]
        x = einops.rearrange(x, "b c d h w -> (b d) c h w", )
        for blk in self.blocks:
            x = blk(x)
        x = self.neck(x.permute(0, 3, 1, 2))
        x = einops.rearrange(x, ("(b d) c h w -> b c d h w"), b = B, d = D//self.depth_downsample_factor)
        return x
