...
class DeformableAttention3D(nn.Module):
    def __init__(
        ...
        *,
        ...
        offset_conv_padding = tuple(
            map(lambda x: (x[0] - x[1]) / 2, zip(offset_kernel_size, downsample_factor))
        )
        assert all(
            [(padding > 0 and padding.is_integer()) for padding in offset_conv_padding]
        )
        ...
        self.to_offsets = nn.Sequential(
            nn.Conv3d(
                ...
                padding=tuple(map(int, offset_conv_padding)),
            ),
            nn.GELU(),
    ...
    def forward(self, x, parq_query_tokens, return_vgrid=True):
        """
        b - batch
        h - heads
        f - frames
        ...
        n_queries = parq_query_tokens.shape[1]


        heads, b, f, h, w, downsample_factor, device = (
            self.heads,
            x.shape[0],
            *x.shape[-3:],
        ...
        query_tokens = (
            parq_query_tokens.permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1) * self.scale
        )
        q, k, v = map(
            lambda t: rearrange(t, "b (h d) ... -> b h (...) d", h=heads),
            (query_tokens, k, v),
        )
        ...
        out = scaled_dot_product_attention(q, k, v, dropout_p=self.dropout_p)
        out = rearrange(out, "b h (f x y) d -> b (h d) f x y", f=n_queries, x=1, y=1)
        out = self.to_out(out)[:, :, :, 0, 0].permute(0, 2, 1)
...
"""
        ...
        query_tokens = parq_query_tokens.permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1)*self.scale
        q, k, v = map(lambda t: rearrange(t, 'b (h d) ... -> b h (...) d', h = heads), (query_tokens, k, v))
        ...
        input()
        out = rearrange(out, 'b h (f x y) d -> b (h d) f x y', f = 8, x = 1, y = 1)
        out = self.to_out(out)[:,:,:,0,0].permute(0, 2, 1)
