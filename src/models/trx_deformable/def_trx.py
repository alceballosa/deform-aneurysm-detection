...
class DeformableTransformer(nn.Module):
    ...
    def forward(
        ...
        """
        Deformable transformer for multiscale deformable attention.

        Parameters
        ----------
        multiscale_feats: List[torch.Tensor]
            List of features from different levels of the backbone.
            Each feature map has shape [B, C, D, H, W].
        multiscale_pos_embs: List[torch.Tensor]
            List of position embeddings for different levels, flattened.
            Shape [N, C] or [B, N, C] where N = D*H*W for the particular level.
