...
def norm_layer2d(norm_type, num_features):
    norm = {
        "BN": nn.BatchNorm2d,
        ...
        "LN": LayerNorm,
        "IN": lambda channels: nn.InstanceNorm2d(num_features=channels, affine=True),
        "": nn.Identity,
        "none": nn.Identity,
...
class ConvBlock(nn.Module):
    def __init__(
        ...
        self.norm = norm_layer2d(norm_type, out_channels)
...
class DownsamplingBlock(nn.Module):
    def __init__(
        ...
        if pool_type == "avg":
            self.down = nn.AvgPool2d(kernel_size=stride, stride=stride)
        else:
            self.down = nn.MaxPool2d(kernel_size=stride, stride=stride)
...
class ClsRegHead(nn.Module):
    def __init__(
        ...
        self.cls_output = nn.Conv2d(feature_size, 1, kernel_size=3, padding=1)
        ...
        self.shape_output = nn.Conv2d(feature_size, 3, kernel_size=3, padding=1)
        ...
        self.offset_output = nn.Conv2d(feature_size, 3, kernel_size=3, padding=1)
