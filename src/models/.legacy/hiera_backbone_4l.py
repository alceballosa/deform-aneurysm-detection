...
def build_backbone(cfg):
    ...
    return Hiera(
        cfg,
        num_classes=1,  # K400 has 400 classes
        in_chans=cfg.DATA.N_CHANNELS,
        input_size=cfg.DATA.PATCH_SIZE,
        q_stride=(2, 2, 2),
        mask_unit_size=(4, 4, 4),
        patch_kernel=(7, 7, 7),
        patch_stride=(2, 2, 2),
        patch_padding=(3, 3, 3),
        sep_pos_embed=False,
...
class Hiera(Base_Backbone):
    ...
    def encode_multiscale_feats(self, x) -> List:
        results = self.forward_hiera(x)
        results = [
            x.permute(0, 4, 1, 2, 3) for x in results
        ]
        return results