...
from src.models.deformable.def_trx import Transformer, build_deformable_transformer
from src.models.deformable import cnn_backbone, unet_backbone, unet_backbone_4l
...
class PARQ_Deformable(nn.Module):
    @configurable
    def __init__(
        self,
        cfg,
        ...
        super(PARQ_Deformable, self).__init__()
        self.cfg = cfg
    ...
    def _forward_network(self, x):
        ...
        multiscale_feats, multiscale_pos_embs = self.backbone(x)
        hs, init_reference_out, inter_references, attn_list = self.transformer.forward(
            multiscale_feats,
            multiscale_pos_embs,
    ...
    def bbox3d_prediction(self, attended_queries, query_pos):
        """
        ...
        """
        dims = self.patch_size
        if attended_queries.dim() == 4:
            queries_list = torch.split(attended_queries, 1, dim=0)
        else:
            queries_list = [attended_queries.unsqueeze(0)]
        ...
        for queries in queries_list:
            ...
            cls_logits = self.mlp_heads["cls_head"](queries).transpose(1, 2)
            center_offset = self.mlp_heads["center_head"](queries).transpose(1, 2)
            ...
            size_normalized = (
                self.mlp_heads["size_head"](queries).transpose(1, 2).sigmoid()
            )
            ...
            box_prediction = {
                ...
                "coord_pos": query_pos,  # use input reference point to match instead of output center
            }
    ...
    def compute_losses(self, output_dict, targets):
        ...
        loss_total = (
            output_dict[-1]["size_normalized"].sum()
            * output_dict[-1]["center_normalized"].sum()
            * output_dict[-1]["pred_logits"].sum()
            * 0
        )
        ...
        for out_dict in output_dict:
            ...
            for i in range(bs):
                ...
                if len(matched_indices[i]) == 0:
                    ...
                    center_predict = out_dict["center_normalized"][i][
                        matched_indices[i][0]
                    ]
                    ...
                    size_predict = out_dict["size_normalized"][i][matched_indices[i][0]]
    ...
    def parse_pred(self, pred_dict):
        ...
        size_predict = pred_dict["size_normalized"]
        center_predict = pred_dict["center_normalized"]
    ...
    def normalize_input_values(self, x: torch.Tensor):
        if self.backbone_type in ["UNET","UNET_4L", "UNET2D", "CNN"]:
