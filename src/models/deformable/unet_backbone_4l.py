...
class UNET_Backbone(Base_Backbone):
    def __init__(
        ...
        self.block11_up = UpsamplingDeconvBlock(
            n_filters[1], n_filters[0], norm_type=norm_type, act_type=act_type
        )
        self.block11_res = LayerBasic(
            1, n_filters[0], n_filters[0], norm_type=norm_type, act_type=act_type, se=se
        )
        self.block11 = LayerBasic(
            2,
            n_filters[0] * 2,
            n_filters[0],
            norm_type=norm_type,
    ...
    def encode_multiscale_feats(self, x) -> List[torch.Tensor]:
        ...
        x_out_third_upsample = x = self.block11(x)
        return [
            x_out_third_upsample,
            x_out_second_upsample,
            x_out_first_upsample,
            x_bottleneck,
        ]
