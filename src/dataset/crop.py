...
class InstanceCrop(object):
    ...
    def __init__(
        ...
        Arguments:
            ...
            tp_ratio: sampling rate for a patch containing at least one lesion
    ...
    def __call__(self, sample):
        ...
        # compute sampling prob for each samples
        ...
        index = np.random.choice(
            np.arange(len(crop_centers)), size=self.sample_num, p=p
        )
    ...
    def make_patch(
        ...
        """
        create a patch with annotation from the patch center
        ...
        # find which annotations is in the patch
        in_idx = []
...
def resample_simg(simg, interp=sitk.sitkBSpline, spacing=[1.0, 0.7, 0.7]):
    ...
    sz1 = simg.GetSize()
    sz2 = (
        int(round(sz1[0] * sp1[0] / new_spacing[0])),
        int(round(sz1[1] * sp1[1] / new_spacing[1])),
        int(round(sz1[2] * sp1[2] / new_spacing[2])),
    )

    new_origin = simg.GetOrigin()
    new_origin = (
        new_origin[0] - sp1[0] / 2 + new_spacing[0] / 2,
        new_origin[1] - sp1[1] / 2 + new_spacing[1] / 2,
