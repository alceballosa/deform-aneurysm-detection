    ...
    for seriesuid in all_seriesuid:
        ...
        if seriesuid in mask_cache["artery"]:
            ...
            brain_mask = sitk.ReadImage(
                str(path_brain / f"{seriesuid.split('.')[0]}/brain.nii.gz")
            )
        ...
        for i, row in preds_seriesuid.iterrows():
            ...
            row["overlap_artery"] = np.sum(
                artery_mask[z:z_2, y:y_2, x:x_2]
            ) / ((x_2 - x) * (y_2 - y) * (z_2 - z))
            row["overlap_vein"] = np.sum(
                vein_mask[z:z_2, y:y_2, x:x_2]
            ) / ((x_2 - x) * (y_2 - y) * (z_2 - z))
