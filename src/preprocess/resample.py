...
def run(im_f, mask_f):
    ...
    if np.abs(space - target_spacing).max() < 0.01:
        ...
        sitk.WriteImage(
            mask, mask_f.replace("internal_label_test", "internal_label_test_0.4")
        )
    else:
        print("resample before", space, image.GetSize())
        image_resampled = resample_simg(
            itkimage=image, newSpacing=target_spacing, label=False
        )
        mask_resampled = resample_simg(
        ...
        sitk.WriteImage(
            image_resampled, im_f.replace("internal_test", "internal_test_0.4")
        )
        sitk.WriteImage(
            mask_resampled,
            mask_f.replace("internal_label_test", "internal_label_test_0.4"),
        )
